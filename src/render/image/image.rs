...
impl ImageData {
    pub fn new(width: u32, height: u32, format: ImageFormat) -> Self {
        let bpp = match format { ImageFormat::RGBA8 | ImageFormat::BGRA8 => 4, _ => 3 };
        ImageData { pixels: vec![0; (width * height * bpp) as usize], width, height, format }
    }

    pub fn from_rgba(width: u32, height: u32, data: Vec<u8>) -> Self {
        ImageData { pixels: data, width, height, format: ImageFormat::RGBA8 }
    }
    ...
    pub fn get_pixel(&self, x: u32, y: u32) -> Option<(u8, u8, u8, u8)> {
        ...
        if idx + 4 > self.pixels.len() { return None; }
    ...
    pub fn bytes(&self) -> &[u8] { &self.pixels }
}
